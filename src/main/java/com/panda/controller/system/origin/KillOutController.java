 ...
 */
@Controller
@RequestMapping(value = "/system/killOut")
public class KillOutController {
    ...
    @RequiresPermissions("killOut:view")//权限管理;
    public String getMachinTeaListKillOut(HttpServletRequest request, Model model) {
        ...
        return "system/origin/getKillOutList";
    }
     ...
     * @return
     */
    @RequestMapping(value = "/editKillOutItem", method = RequestMethod.GET)
    @ResponseBody
    public Object editKillOutItem(HttpServletRequest request, String id) {
        message = false;
        data = null;
        if (!id.isEmpty()) {
            try {
     ...
     * @return
     */
    @RequestMapping(value = "/saveOrUpdateKillOut", method = RequestMethod.POST)
    @ResponseBody
    public Object saveOrUpdateKillOut(HttpServletRequest request, MachinTea machinTea, String save) {
        Users user = (Users) SecurityUtils.getSubject().getPrincipal();
        message = false;
        data = null;
        try {
            if (machinTea != null) {
                if (save.equals("edit")) {
                    ...
                    int insert = machinTeaService.saveOrUpdateKillOut(machinTea);
                    switch (insert) {
                        case 200:
                            message = true;
     ...
     * @return
     */
    @RequestMapping(value = "/delKillOutItem", method = RequestMethod.GET)
    @ResponseBody
    public Object delKillOutItem(HttpServletRequest request, String id) {
        message = false;
        data = null;
        try {
            if (!id.isEmpty()) {
                int result = machinTeaService.delWitherItem(id, "killOut");
                switch (result) {
                    case 200:
                        message = true;
                        data = ResultStateUtil.SUCCESS_ABNORMAL;
