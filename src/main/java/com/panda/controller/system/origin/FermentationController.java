 ...
 */
@Controller
@RequestMapping(value = "/system/fermentation")
public class FermentationController {
    ...
    @RequiresPermissions("fermentation:view")//权限管理;
    public String getMachinTeaListFermentation(HttpServletRequest request, Model model) {
        ...
        return "system/origin/getFermentationList";
    }
     ...
     * @return
     */
    @RequestMapping(value = "/getFermentationDataList", method = RequestMethod.POST)
    @ResponseBody
    public Object getFermentationDataList(HttpServletRequest request, String datatable) {
        message = false;
        data = null;
        try {
            Map query = new HashMap();
            query.put("dicMacValue", "fermentation");
     ...
     * @return
     */
    @RequestMapping(value = "/editFermentationItem", method = RequestMethod.GET)
    @ResponseBody
    public Object editFermentationItem(HttpServletRequest request, String id) {
        message = false;
        data = null;
        if (!id.isEmpty()) {
            try {
     ...
     * @return
     */
    @RequestMapping(value = "/saveOrUpdateFermentation", method = RequestMethod.POST)
    @ResponseBody
    public Object saveOrUpdateFermentation(HttpServletRequest request, MachinTea machinTea, String save) {
        Users user = (Users) SecurityUtils.getSubject().getPrincipal();
        message = false;
        data = null;
        try {
            if (machinTea != null) {
                if (save.equals("edit")) {
                    ...
                    int insert = machinTeaService.saveOrUpdateFermentation(machinTea);
                    switch (insert) {
                        case 200:
                            message = true;
     ...
     * @return
     */
    @RequestMapping(value = "/delFermentationItem", method = RequestMethod.GET)
    @ResponseBody
    public Object delFermentationItem(HttpServletRequest request, String id) {
        message = false;
        data = null;
        try {
            if (!id.isEmpty()) {
                int result = machinTeaService.delWitherItem(id, "fermentation");
                switch (result) {
                    case 200:
                        message = true;
                        data = ResultStateUtil.SUCCESS_ABNORMAL;
