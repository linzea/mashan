 ...
 */
@Controller
@RequestMapping(value = "/system/dryTea")
public class DryTeaController {
    ...
    @RequiresPermissions("dryTea:view")//权限管理;
    public String getMachinTeaListKillOut(HttpServletRequest request, Model model) {
        ...
        return "system/origin/getDryTeaList";
    }
     ...
     * @return
     */
    @RequestMapping(value = "/getDryTeaDataList", method = RequestMethod.POST)
    @ResponseBody
    public Object getDryTeaDataList(HttpServletRequest request, String datatable) {
        message = false;
        data = null;
        try {
            Map query = new HashMap();
            query.put("dicMacValue", "dryTea");
     ...
     * @return
     */
    @RequestMapping(value = "/editDryTeaItem", method = RequestMethod.GET)
    @ResponseBody
    public Object editDryTeaItem(HttpServletRequest request, String id) {
        message = false;
        data = null;
        if (!id.isEmpty()) {
            try {
     ...
     * @return
     */
    @RequestMapping(value = "/saveOrUpdateDryTea", method = RequestMethod.POST)
    @ResponseBody
    public Object saveOrUpdateDryTea(HttpServletRequest request, MachinTea machinTea, String save) {
        Users user = (Users) SecurityUtils.getSubject().getPrincipal();
        message = false;
        data = null;
        try {
            if (machinTea != null) {
                if (save.equals("edit")) {
                    ...
                    int insert = machinTeaService.saveOrUpdateDryTea(machinTea);
                    switch (insert) {
                        case 200:
                            message = true;
     ...
     * @return
     */
    @RequestMapping(value = "/delDryTeaItem", method = RequestMethod.GET)
    @ResponseBody
    public Object delDryTeaItem(HttpServletRequest request, String id) {
        message = false;
        data = null;
        try {
            if (!id.isEmpty()) {
                int result = machinTeaService.delWitherItem(id, "DryTea");
                switch (result) {
                    case 200:
                        message = true;
                        data = ResultStateUtil.SUCCESS_ABNORMAL;
