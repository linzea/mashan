 ...
 */
@Controller
@RequestMapping(value = "/system/roasting")
public class RoastingController {
    ...
    @RequiresPermissions("roasting:view")//权限管理;
    public String getMachinTeaListRoasting(HttpServletRequest request, Model model) {
        ...
        return "system/origin/getRoastingList";
    }
     ...
     * @return
     */
    @RequestMapping(value = "/getRoastingDataList", method = RequestMethod.POST)
    @ResponseBody
    public Object getRoastingDataList(HttpServletRequest request, String datatable) {
        message = false;
        data = null;
        try {
            Map query = new HashMap();
            query.put("dicMacValue", "roasting");
     ...
     * @return
     */
    @RequestMapping(value = "/editRoastingItem", method = RequestMethod.GET)
    @ResponseBody
    public Object editRoastingItem(HttpServletRequest request, String id) {
        message = false;
        data = null;
        if (!id.isEmpty()) {
            try {
     ...
     * @return
     */
    @RequestMapping(value = "/saveOrUpdateRoasting", method = RequestMethod.POST)
    @ResponseBody
    public Object saveOrUpdateRoasting(HttpServletRequest request, MachinTea machinTea, String save) {
        Users user = (Users) SecurityUtils.getSubject().getPrincipal();
        message = false;
        data = null;
        try {
            if (machinTea != null) {
                if (save.equals("edit")) {
                    ...
                    int insert = machinTeaService.saveOrUpdateRoasting(machinTea);
                    switch (insert) {
                        case 200:
                            message = true;
     ...
     * @return
     */
    @RequestMapping(value = "/delRoastingItem", method = RequestMethod.GET)
    @ResponseBody
    public Object delRoastingItem(HttpServletRequest request, String id) {
        message = false;
        data = null;
        try {
            if (!id.isEmpty()) {
                int result = machinTeaService.delWitherItem(id, "roasting");
                switch (result) {
                    case 200:
                        message = true;
                        data = ResultStateUtil.SUCCESS_ABNORMAL;
