 ...
 */
@Controller
@RequestMapping(value = "/system/titian")
public class TitianController {
    ...
    @RequiresPermissions("titian:view")//权限管理;
    public String getMachinTeaListTitian(HttpServletRequest request, Model model) {
        ...
        return "system/origin/getTitianList";
    }
     ...
     * @return
     */
    @RequestMapping(value = "/getTitianDataList", method = RequestMethod.POST)
    @ResponseBody
    public Object getTitianDataList(HttpServletRequest request, String datatable) {
        message = false;
        data = null;
        try {
            Map query = new HashMap();
            query.put("dicMacValue", "titian");
     ...
     * @return
     */
    @RequestMapping(value = "/editTitianItem", method = RequestMethod.GET)
    @ResponseBody
    public Object editTitianItem(HttpServletRequest request, String id) {
        message = false;
        data = null;
        if (!id.isEmpty()) {
            try {
     ...
     * @return
     */
    @RequestMapping(value = "/saveOrUpdateTitian", method = RequestMethod.POST)
    @ResponseBody
    public Object saveOrUpdateTitian(HttpServletRequest request, MachinTea machinTea, String save) {
        Users user = (Users) SecurityUtils.getSubject().getPrincipal();
        message = false;
        data = null;
        try {
            if (machinTea != null) {
                if (save.equals("edit")) {
                    ...
                    int insert = machinTeaService.saveOrUpdateTitian(machinTea);
                    switch (insert) {
                        case 200:
                            message = true;
     ...
     * @return
     */
    @RequestMapping(value = "/delTitianItem", method = RequestMethod.GET)
    @ResponseBody
    public Object delTitianItem(HttpServletRequest request, String id) {
        message = false;
        data = null;
        try {
            if (!id.isEmpty()) {
                int result = machinTeaService.delWitherItem(id, "Titian");
                switch (result) {
                    case 200:
                        message = true;
                        data = ResultStateUtil.SUCCESS_ABNORMAL;
