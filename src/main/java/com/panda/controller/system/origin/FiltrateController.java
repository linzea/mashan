 ...
 */
@Controller
@RequestMapping(value = "/system/filtrate")
public class FiltrateController {
    ...
    @RequiresPermissions("filtrate:view")//权限管理;
    public String getMachinTeaListFiltrate(HttpServletRequest request, Model model) {
        ...
        return "system/origin/getFiltrateList";
    }
     ...
     * @return
     */
    @RequestMapping(value = "/getFiltrateDataList", method = RequestMethod.POST)
    @ResponseBody
    public Object getFiltrateDataList(HttpServletRequest request, String datatable) {
        message = false;
        data = null;
        try {
            Map query = new HashMap();
            query.put("dicMacValue", "filtrate");
     ...
     * @return
     */
    @RequestMapping(value = "/editFiltrateItem", method = RequestMethod.GET)
    @ResponseBody
    public Object editFiltrateItem(HttpServletRequest request, String id) {
        message = false;
        data = null;
        if (!id.isEmpty()) {
            try {
     ...
     * @return
     */
    @RequestMapping(value = "/saveOrUpdateFiltrate", method = RequestMethod.POST)
    @ResponseBody
    public Object saveOrUpdateFiltrate(HttpServletRequest request, MachinTea machinTea, String save) {
        Users user = (Users) SecurityUtils.getSubject().getPrincipal();
        message = false;
        data = null;
        try {
            if (machinTea != null) {
                if (save.equals("edit")) {
                    ...
                    int insert = machinTeaService.saveOrUpdateFiltrate(machinTea);
                    switch (insert) {
                        case 200:
                            message = true;
     ...
     * @return
     */
    @RequestMapping(value = "/delFiltrateItem", method = RequestMethod.GET)
    @ResponseBody
    public Object delFiltrateItem(HttpServletRequest request, String id) {
        message = false;
        data = null;
        try {
            if (!id.isEmpty()) {
                int result = machinTeaService.delWitherItem(id, "Filtrate");
                switch (result) {
                    case 200:
                        message = true;
                        data = ResultStateUtil.SUCCESS_ABNORMAL;
